...
-- CURRENT_DATE : 접속한 계정
-- CURRENT_TIMESTAMP : 현재 날짜 및 로컬 시간을 리턴
...
-- ORACLE은 자료형이 맞지 않은 상황이라도 작성된 값의 형태가 요구하는 자료형의 형태를 띄고 있으면, 자동으로 형변환(PARSING)을 진행한다
...
/* 숫자 -> 문자열 변환 시 발생할 수 있는 문제 상황 : 지정된 포맷의 칸 수가 변환하려는 숫자 자릿수 보다 적은 경우
...
-----------------------------------------------------------------------------------
/* <그룹 함수> : N개의 행이 컬럼 값을 전달하여 1개의 결과가 반환(그룹의 수가 늘어나면서 그룹의 수 만큼 결과를 반환)
