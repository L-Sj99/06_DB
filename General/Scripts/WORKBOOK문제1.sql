 ...
 * 단, 출력 헤더에는 "학생이름", "학번", "거주지 주소"가 출력 되도록 한다 */
...
WHERE STUDENT_NO IN (
	...
	WHERE SUBSTR(STUDENT_ADDRESS, 1, 3) = '경기도'
	OR SUBSTR(STUDENT_ADDRESS, 1, 2) = '강원')
	AND STUDENT_NO NOT LIKE ('A______')
ORDER BY STUDENT_NAME ASC, STUDENT_NO ASC, STUDENT_ADDRESS ASC;
 ...
 * (FM : 조회 결과 양쪽 공백 제거) */
SELECT TERM_NO, CLASS_NO, STUDENT_NO
FROM TB_GRADE;
...
SELECT COACH_PROFESSOR_NO, PROFESSOR_NAME
FROM TB_STUDENT S
...
SELECT STUDENT_NAME, STUDENT_ADDRESS, DEPARTMENT_NO
FROM TB_STUDENT
WHERE DEPARTMENT_NO = (
...
SELECT STUDENT_NAME, STUDENT_NO, POINT
FROM TB_STUDENT S
JOIN TB_GRADE G ON (S.STUDENT_NO = G.STUDENT_NO);
WHERE ;
...
FROM TB_DEPARTMENT D
JOIN TB_CLASS C ON (D.DEPARTMENT_NO = C.DEPARTMENT_NO)
JOIN TB_GRADE G ON (D.CLASS_NO = G.CLASS_NO)
WHERE CATEGORY = (
	SELECT CATEGORY
	FROM TB_DEPARTMENT
