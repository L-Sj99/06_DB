 ...
 * 트리거(TRIGGER), 프로시져(PROCEDURE), 함수(FUNCTION), 동의어(SYNONYM), 사용자(USER)
 */
 ...
 *   UTF-8은 영어/숫자 1BYTE,  한글 3BYTE로 저장하기 때문에 한글 데이터 저장 시 크기 계산을 주의해야함
 * 
...
-- 오류 보고에 나타나는 SYS_C008635 같은 제약 조건명으로 해당 제약 조건이 설정된 테이블명, 컬럼, 제약 조건 타입 조회 
...
---------------------------------------
-- UNIQUE 복합키 : 두 개 이상의 컬럼을 묶어서 하나의 UNIQUE 제약조건을 설정함
