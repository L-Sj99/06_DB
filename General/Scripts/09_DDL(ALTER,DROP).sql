...
-- ADD 방법
ALTER TABLE DEPT_COPY
ADD CONSTRAINT DEPT_COPY_TITLE_NN NOT NULL(DAPT_TITLE);
...
ALTER TABLE DEPT_COPY
MODIFY DEPT_TITLE NOT NULL; -- NULL 비허용
...
SELECT * FROM DEPT_COPY;
--> 개로 추가된 LNAME 컬럼에 기본값 '한국'이 자동으로 셋팅됨
...
-- DEPT_COPY 테이블의 LNAME 컬럼 값을 기본 값인 '대한민국'으로 모두 변경
UPDATE DEPT_COPY
SET LNAME = DEFAULT;
...
-- DML을 수행하여 트랜잭션에 변경사항이 저장된 상태에서 COMMIT/ROLLBACK 없이 DDL 구문을 수행하게되면
-- DDL 수행과 동시에 선행 DML이 자동으로 COMMIT 되어버림

-- DEPT_COPY 테이블에 DEPT_ID 컬럼 값으로 'D0' 추가 INSERT(DML) 수행
...
SELECT * FROM DEPT_COPY;
-- 00942: 테이블 또는 뷰가 존재하지 않습니다

-- ** 관계가 형성된 테이블 삭제 **
-- TB1 테이블 생성
CREATE TABLE TB1(
	TB1_PK NUMBER PRIMARY KEY, TB1_COL NUMBER);

-- TB2 테이블 생성(TB1과 관계 설정(FK)
CREATE TABLE TB2(
	TB2_PK NUMBER PRIMARY KEY, TB2_COL NUMBER REFERENCES TB1(TB1_PK));

-- TB1(부모) 테이블 삭제
DROP TABLE TB1;
-- ORA-02449: 외래 키에 의해 참조되는 고유/기본 키가 테이블에 있습니다
-- == PK 컬림이 다른 테이블에 참조를 당하고 있다
...
-- TB1 테이블 삭제 + TB2 테이블 FK 제약 조건도 같이 삭제됨
...
-- 복사한 테이블에 PK 추가
ALTER TABLE DEPT_COPY
ADD CONSTRAINT DEPT_COPT_PK PRIMARY KEY(DEPT_ID);
...
ALTER TABLE DEPT_COPY
RENAME CONSTRAINT DEPT_COPT_PK TO PK_DCOPY;
