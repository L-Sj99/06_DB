...
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); --> DEPT_CODE와 DEPT_ID 값이 같은 행을 한 행으로 합쳐라

/*  JOIN은 단순히 테이블을 두개를 붙이는 것이 아닌 기준 삼은 테이블의 한 컬럼을 지정해
 * 	다른 테이블의 한 컬럼과 같은 행을 찾아 기준 테이블 옆에 한 행씩 붙여나감 
 */
...
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID); -- 21행, 하동운과 이오리 제외
--> DEPT_CODE, DEPT_ID가 일치하는 행끼리 한 행으로 합쳐짐 단, 일치하는 값이 없을 경우 JOIN 결과에서 제외
--> INNER JOIN
...
-- 부서명이 '해외영업2부'인 사원의 사번(EMP_ID), 이름(EMP_NAME), 부서명(DEPT_TITLE)인 사번 오름차순으로 조회
SELECT * FROM DEPARTMENT;
...
-- 오라클 -> 별칭 사용 : 테이블 별로 별칭을 등록할 수 있음.
SELECT EMP_ID, EMP_NAME, E.JOB_CODE, JOB_NAME -- (JOB_CODE가 어떤 테이블의 컬럼을 조회할지 별칭을 작성)
...
-- 두 테이블의 지정하는 컬럼값이 일치하지 않는 행도 조인에 포함을 시킴 *반드시 OUTER JOIN임을 명시*
...
-- INNER JOIN 결과에 포함되지 않았던 DEPARTMENT의 D3, D4, D7도 결과에 포함
...
--> EMPLOYEE 테이블의 하동운, 이오리, DEPARTMENT 테이블의 D3, D4, D7 전부 JOIN 결과에 포함
...
--------------------------------------------------------------------------
-- 4. 비등가 조인(NON EQUAL JOIN) : '='(등호)를 사용하지 않는 조인문
--  지정한 컬럼 값이 일치하는 경우가 아닌, 값의 범위에 포함되는 행들을 연결하는 방식

-- 사원의 급여가 SAL_LEVEL에 작성된 최소(MIN_SAL) ~ 최대(MAX_SAL) 범위의 급여가 맞을 때만 결과에 포함하겠다는 JOIN
--> 사원이 자신의 급여 레벨에 맞는 돈을 잘 받고 있는지 확인
SELECT E.EMP_NAME, E.SAL_LEVEL, E.SALARY, S.MIN_SAL, S.MAX_SAL
FROM EMPLOYEE E
JOIN SAL_GRADE S ON (E.SALARY BETWEEN S.MIN_SAL AND S.MAX_SAL); -- 23행
...
-- EMPLOYEE 테이블에서 모든 사원의 이름과 사수 번호(MANAGER_ID), 사수 이름(MANAGER_NAME) 조회
...
-------------------------------------------------------------------------
-- 6. 자연 조인(NATURAL JOIN) : 동일한 타입과 이름을 가진 컬럼이 있는 테이블 간의 조인을 간단히 표현하는 방법
-- 반드시 두 테이블 간의 동일한 컬럼명, 타입을 가진 컬럼이 필요 -> 없을 경우 교차조인이 됨
...
FROM EMPLOYEE
NATURAL JOIN JOB;
--> 두 테이블에서 같은 컬럼명, 자료형을 지닌 JOB_CODE 컬럼 기준으로 JOIN이 진행

/* NATURAL JOIN 실패 -> 같은 컬럼명, 자료형이 없을 경우 JOIN 실패
 * --> 결과로 CROSS JOIN 형태가 조회됨
 */
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
...
-- 직급이 대리이면서 아시아 지역에 근무하는 직원 조회 사번, 이름, 직급명, 부서명, 근무지역명, 급여를 조회하세요
...
-- 3. 해외영업 1부, 2부에 근무하는 사원의 사원명, 직급명, 부서코드, 부서명 중 사번 오름차순 조회
...
-- 6. 급여등급별 최소급여(MIN_SAL)를 초과해서 받는 직원들의 사원명, 직급명, 급여, 연봉(보너스포함)을 조회하시오
--연봉에 보너스포인트를 적용하시오
