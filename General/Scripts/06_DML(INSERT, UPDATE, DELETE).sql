...
-- SELECT도 DML에 포함되긴 하지만 DQL이라는 것으로 분리해서 보기도 함
...
ROLLBACK; --> COMMIT 되지 않은 DML (INSERT, UPDATE, DELET) 취소하기
	-- == DML 수행 전으로 되돌리기
---------------------------------------
...
VALUES ('장채현', 900, '001123-4345678', 'jang_ch@kh.or.kr', '01012341234',
'D1', 'J7', 'S3', 4300000);
SELECT * FROM EMPLOYEE2
WHERE EMP_ID = 900;
...
-- 서브쿼리를 이용해서 여러행 INSERT 하기 --> 서브쿼리 결과(RESULT SET)를 통째로 삽입하기
INSERT INTO EMP_01
(SELECT 
	EMP_ID,
	EMP_NAME,
...
-- 결과 확인
SELECT * FROM DEPARTMENT2; -- D0 / 전략기획2팀 / L1
...
-- EMPLOYEE2 테이블에서 평상시 유재식 사원을 부러워하던 방명수 사원의 급여와 보너스율을 유재식 사원과 동일하게 변경해 주기로 했다
...
-- EMPLOYEE2 테이블에서 방명수 사원의 급여 인상 소식을 전해들은 다른 멤버들이 단체로 파업을 진행했다
-- 노옹철, 전형돈, 정중하, 하동운 사원의 급여와 보너스를 유재식 사원의 급여와 보너스와 같게 변경하는 UPDATE문을 작성하시오
...
VALUES (999, '곽두원', '561016-1234567', 'kwack_dw@kh.or.kr',
        ...
        SYSDATE, NULL, DEFAULT); -- 1행 추가
       
SELECT * FROM EMP_M01; -- 23행
SELECT * FROM EMP_M02; -- 5행
--> EMP_MO2는 MO1 테이블과 4개의 행이 중복되고, 1개의 행이 다름
...
SELECT * FROM EMPLOYEE2; -- 조회결과 없음

ROLLBACK;
---------------------------------------------------------------------------------------------
-- 5. TRUNCATE (DDL 입니다! DML 아닙니다!)
-- 테이블의 전체 행을 삭제하는 DDL DELETE보다 수행속도가 더 빠르다.
