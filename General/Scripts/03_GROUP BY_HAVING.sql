...
FROM EMPLOYEE
GROUP BY DEPT_CODE; -- 같은 DEPT_CODE끼리 묶음
-- D1, D2, D5, D6, D8, D9, NULL

-- EMPLOYEE 테이블에서 부서코드, 부서 별 급여의 합계, 부서 별 급여의 평균(정수처리), 인원 수를 조회하고 부서 코드 순으로 정렬
...
-- EMPLOYEE 테이블에서 부서코드와 부서별 보너스를 받는 사원의 수를 조회하고 부서코드 순으로 정렬
SELECT DEPT_CODE, COUNT(*) -- 조회 결과에 포함 될 컬럼 지정 (4)
FROM EMPLOYEE -- 어떤 테이블에서 (1)
WHERE BONUS IS NOT NULL -- 어떤 행만 조회할지 지정 (2)
GROUP BY DEPT_CODE -- WHERE 결과에서 그룹 구분 (3)
ORDER BY DEPT_CODE ASC; -- 조회 결과 정렬 (5)


-- EMPLOYEE 테이블에서 성별과 성별 별 급여 평균(정수처리), 급여 합계, 인원 수 조회하고 인원수로 내림차순 정렬
...
FROM EMPLOYEE
GROUP BY DEPT_CODE, JOB_CODE -- 1) 부서 코드 2) 직급 코드
ORDER BY DEPT_CODE ASC;

-- EMPLOYEE 테이블에서 부서 별로 급여 등급이 같은 직원의 수를 조회하고 부서코드, 급여 등급 오름차순으로 정렬
...
/* WHERE와 HAVING의 차이
 * WHERE : FROM절에서 지정된 테이블 전체 행 중 조건에 맞는 행만 골라내는 구문
 * HAVING : GROUP BY를 통해 만들어진 그룹 중 조건에 맞는 그룹만 골라내는 구문
 */
...
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE ASC;*/
-- 개인 급여가 350만 이상인 사람들의 부서별 평균(잘못된 구문)
...
-- 그룹 별 급여 평균이 350만 이상인 그룹만 조회(맞는 SQL)

-- 부서별 그룹의 급여 합계 중 1천만원을 초과하는 부서코드와 급여 합계 조회 부서 코드 순으로 정렬
SELECT DEPT_CODE, SUM(SALARY)
FROM EMPLOYEE
GROUP BY DEPT_CODE
...
-- 1. EMPLOYEE 테이블에서 각 부서별 가장 높은 급여, 가장 낮은 급여를 조회하여 부서 코드 오름차순으로 정렬하세요.
SELECT DEPT_CODE, MAX(SALARY), MIN(SALARY)
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE ASC;

-- 2.EMPLOYEE 테이블에서 각 직급별 보너스를 받는 사원의 수를 조회하여 직급코드 오름차순으로 정렬하세요
SELECT JOB_CODE, COUNT(*)
FROM EMPLOYEE
WHERE BONUS IS NOT NULL
GROUP BY JOB_CODE
ORDER BY JOB_CODE ASC; -- 4행 조회(0명은 조회 결과에 포함X)

/* 통계적인 측면에서는 아래 방법이 더 좋음 */
-- COUNT(컬럼명) : 해당 컬럼 내 값이 행에 몇개 있는지 카운트 --> NULL이면 카운트에서 제외
SELECT JOB_CODE, COUNT(BONUS)
FROM EMPLOYEE
GROUP BY JOB_CODE
ORDER BY JOB_CODE ASC; -- 7행 조회

-- 3.EMPLOYEE 테이블에서 부서별 80년대생의 급여 평균이 400만 이상인 부서를 조회하여 부서 코드 오름차순으로 정렬하세요
SELECT DEPT_CODE, FLOOR(AVG(SALARY)) "급여 평균"
FROM EMPLOYEE
WHERE SUBSTR(EMP_NO, 1, 1) = '8' -- EMP_NO LIKE ('8_______%')
GROUP BY DEPT_CODE
...
/* 집계함수(ROLLUP, CUBE)
 * 그룹 별 산출한 결과 값의 집계를 계산하는 함수 GOURP BY 절에만 작성하는 함수
...
-- UNION : 여러개의 쿼리 결과를 하나로 합치는 연산자(중복된 영역을 제외하여 하나로 합친다)
...
--> 김해술, 박나라, 윤은해, 하이유 4행씩 조회 됨
-- 교집합 심봉선, 대북혼을 제외한 앞쪽 SELECT의 RESULT SET이 조회됨
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE 
WHERE DEPT_CODE = 'D5'
MINUS
