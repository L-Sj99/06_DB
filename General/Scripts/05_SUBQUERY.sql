...
/* 기본 적으로 "서브 쿼리"를 먼저 해석 후 서브쿼리 결과를 이용해서 "메인 쿼리" 해석
...
--    서브쿼리의 조회 결과 값의 개수가 1개인 서브쿼리 WHERE절에서 단일행 서브쿼리 앞에는 비교 연산자 사용
--    <, >, <=, >=, =, !=/^=/<>
...
-- 2) 부서별 급여 합이 21760000원 부서의 부서명과 급여 합 조회
...
-- 부서별 인원수가 가작 정은 부서의 부서명, 인원 수 조회
...
-- 인라인 뷰 : FROM절에 사용된 서브쿼리 -> 서브쿼리 결과가 테이블처럼 인식
SELECT 이름, 급여
FROM (SELECT EMP_NAME 이름, SALARY 급여
FROM EMPLOYEE)
WHERE 급여 >= 4000000;
/* 메인쿼리 해석 1순위인 FROM절에 작성된 서브쿼리 결과 컬럼명이 "급여"이기 때문에
 * 메인쿼리 해석 2순위인 WHERE
 * 메인쿼리 해석 3순위인 SELECT절에서도 똑같이 "급여"라고 컬럼명을 작성해야함
 */
-------------------------------------------------------------------------
/* 2. 다중행 서브쿼리 (MULTI ROW SUBQUERY) : 서브쿼리의 조회 결과 값의 개수가 여러행일 때 
...
-- 부서별 최고 급여를 받는 직원의 이름, 직급, 부서코드, 급여를 부서 순으로 정렬하여 조회
...
-- 대리 직급의 직원들 중에서 과장 직급의 최소 급여보다 많이 받는 직원의 사번, 이름, 직급, 급여를 조회하세요
-- 단, > ANY 혹은 < ANY 연산자를 사용하세요

-- > ANY, < ANY : 여러개의 결과값 중에서 하나라도 큰 / 작은 경우 가장 작은 값보다 큰가? / 가장 큰 값 보다 작은가?
...
WHERE NATIONAL_CODE = 'KO'));
-----------------------------------------------------------------------
-- 3. 다중열 서브쿼리 (단일행 = 결과값은 한 행) : 서브쿼리 SELECT 절에 나열된 컬럼 수가 여러개 일 때
...
----------------------------------------------------------------------
-- 4. 다중행 다중열 서브쿼리 : 서브쿼리 조회 결과 행 수와 열 수가 여러개 일 때
...
-- 상관 서브쿼리는 메인쿼리 1행 해석 -> 서브쿼리 수행 -> 다시 메인쿼리 1행 해석 -> 서브쿼리 수행…(메인쿼리 모든 행 조회할 때까지 반복) 
...
SELECT '있음' -- 서브쿼리에서 조회되는 컬럼 값은 중요한게 아님 조회되는 데이터가 있는지 없는지가 중요
FROM EMPLOYEE "SUB"
...
WHERE SUB.JOB_CODE = MAIN.JOB_CODE); -- 먼저 해석된 MAIN 쿼리의 1개 행에 JOB_CODE값을 얻어와 서브쿼리 해석에 사용

-- 부서별 입사일이 가장 빠른 사원의
--    사번, 이름, 부서명(NULL이면 '소속없음'), 직급명, 입사일을 조회하고 입사일이 빠른 순으로 조회하세요
--    단, 퇴사한 직원은 제외하고 조회하세요
SELECT EMP_ID, EMP_NAME, NVL(DEPT_TITLE, '소속없음') AS DEPT_TITLE, JOB_NAME, HIRE_DATE
FROM EMPLOYEE "MAIN"
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB USING(JOB_CODE)
WHERE HIRE_DATE = (
-- 메인쿼리 1행을 해석했을 때 조회되는 행 중에서 DEPT_CODE 값을 얻어와 서브쿼리에서
-- 해당 DEPT_CODE가 일치하는 사원들 중 가장 빠른 입사일을 조회
SELECT MIN(HIRE_DATE)
FROM EMPLOYEE "SUB"
WHERE NVL("SUB".DEPT_CODE, '소속없음') = NVL("MAIN".DEPT_CODE, '소속없음')
--> NULL은 비교가 안되지 때문에 가능한 형태로 변환
AND ENT_YN != 'Y' -- 퇴사자 제외
--> 서브쿼리에서 조회된 결과를 다시 메인쿼리로 넘겨 메인쿼리 WHERE절 조건을 충족하는지 확인
);
...
--    SELECT절에 사용되는 서브쿼리 결과로 1행만 반환(SQL에서 단일 값을 가르켜 '스칼라'라고 함)
...
SELECT EMP_ID "사번", EMP_NAME "이름", MANAGER_ID "사수 사번", NVL(
	(SELECT EMP_NAME
	FROM EMPLOYEE "SUB"
	WHERE SUB.EMP_ID = MAIN.MANAGER_ID), '없음')AS "사수명"
FROM EMPLOYEE "MAIN"
ORDER BY EMP_ID ASC;

-----------------------------------------------------------------------
-- 7. 인라인 뷰(INLINE-VIEW) --> SELECT문에서 조회되는 가상 테이블
/* VIEW (객체) : 보여지는 객체, 조회용도의 가상 테이블
...
FROM EMPLOYEE
WHERE ROWNUM <= 5
ORDER BY SALARY DESC; --> 조회결과가 잘못됨, EMPLOYEE 테이블 위에서 5명만 조회, 정렬된 결과
...
FROM 
	(SELECT EMP_NAME, SALARY
	 FROM EMPLOYEE
	 ORDER BY SALARY DESC) --> 서브쿼리 결과 (23행 2열)를 메인쿼리의 테이블로 인식
WHERE ROWNUM <= 5; --> 정렬된 서브쿼리 결과에서 1, 2, 3, 4, 5 행만 조회

-- 급여 평균이 3위 안에 드는 부서의 부서코드와 부서명, 평균급여를 조회
SELECT ROWNUM "순위", "부서 코드", "부서명", "급여 평균"
FROM
	(SELECT NVL(DEPT_CODE, '없음') "부서 코드", NVL(DEPT_TITLE, '부서명 없음') "부서명",
...
-- 1) RANK() OVER : OVER() 괄호에 작성된 정렬 기준대로 정렬 후 순위 지정
-- 단, 값의 크기가 같다면 공동 순위 지정, 지정된만큼 순위 건너뛰기
SELECT RANK() OVER(ORDER BY SALARY DESC) "순위", --> SALARY 내림차순으로 정렬하고 순위를 지정함
			 EMP_NAME, SALARY
FROM EMPLOYEE;
...
-- 2) DENSE_RANK() OVER : 공동 순위 지정 후 순위 건너뛰기를 하지 않음
SELECT DENSE_RANK() OVER(ORDER BY SALARY DESC) "순위", EMP_NAME, SALARY
FROM EMPLOYEE;

------------------------------------------------------------------------
/* ROWNUM 사용 시 주의사항
 * ROWNUM이 WHERE절에 사용되는 경우 항상 범위에 1부터 연속적인 범위가 포함되어야만 한다
 * --> ROWNUM은 RESULT SET 완성 후 적용되는 가상 컬럼이라서 정해진 규칙(1부터 1씩 증가)을 만족하지 못하면 사용 불가
 */
...
FROM
	...
	FROM EMPLOYEE)
-- WHERE ROWNUM BETWEEN 3 AND 7; -- 3행 이상 7행 이하 == 안됨
...
-- 사번, 이름, 부서명(NULL이면 '소속없음'), 직급명, 입사일, 입사일이 빠른 순으로 조회하시오
-- 단, 퇴사한 직원은 제외하고 조회

-- 1) GROUP BY 이용한 방법(정확도 떨어짐)
SELECT EMP_ID, EMP_NAME,
			 NVL(DEPT_TITLE, '소속 없음') DEPT_TITLE, JOB_NAME, HIRE_DATE
FROM EMPLOYEE E
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE)
WHERE HIRE_DATE IN
