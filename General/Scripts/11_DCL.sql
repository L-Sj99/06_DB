 ...
 * - 계정별로 DB 또는 DB 객체에 대한 접근(제어) 권한을 부여(GRANT), 회수(REVOKE)하는 언어
 */
...
/* TEST_USER 계정에는 데이터 베이스 접속 권한인 CREATE SESSION만 부여됨
 * -> 객체를 다루거나 생성하는 권한이 하나도 없어서 할 수 있는게 없음
 * --> 객체를 생성할 수 있는 공강(TABLESPACE) 할당 + 기본 객체 제어 권한 부여
 */
...
/* 객체 권한 테스트
 * KH 계정이 가지고 있는 테이블 중 EMPLOYEE 테이블에 대한 SELECT 권한을
 * TEST_USER 계정에 부여하기
 */
 ...
 */
SELECT * FROM KH_LSJ.EMPLOYEE;

-- 다른 테이블 조회도 되는지 확인
SELECT * FROM KH_LSJ.DEPARTMENT;
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다
--> 조회 권한이 없어서 테이블 존재 여부조차 모르기 떄문에 발생하는 에러
...
SELECT * FROM KH_LSJ.EMPLOYEE; -- 조회 불가
...
DROP USER TEST_USER;
--> ORA-01940: 현재 접속되어 있는 사용자는 삭제할 수 없습니다 == 연결 해제하고 하면 됨
--> Database Navigator에서 TEST_USER 계정에 접속 중이라서 삭제 불가 --> 연결 종료