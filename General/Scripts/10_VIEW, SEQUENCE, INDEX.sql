...
-- V_EMP에서 대리 직원들의 이름 오름차순 조회
-- (VIEW 조회 결과로 보이는 컬럼명을 이용해야됨)
SELECT * FROM V_EMP
WHERE 직급명 = '대리'
ORDER BY 이름 ASC;
...
-- DEPARTMENT 체이블을 복사한 DEPT_COPY2 생성
CREATE TABLE DEPT_COPY2
AS SELECT * FROM DEPARTMENT;
--> 만들어진 DEPT_COPY2 테이블을 확인해보면 DEPT_TITLE 컬럼만 NULL이 허용되어 있음

-- DEPT_COPY2 테이블에서 DEPT_ID, LOCATION_ID 컬럼만 이용해서 V_DCOPY2 VIEW 생성
...
-- V_DOPY2 VIEW를 이용해서 INSERT 수행
INSERT INTO V_DCOPY2
VALUES('D0', 'L2');
--> 가상 테이블 VIEW인데 INSERT가 성공
 ...
 * 
 * 2) 시퀀스명.CURRVAL : 현재 시퀀스 번호를 얻어옴. 단, 시퀀스가 생성 되자마자 호출할 경우 오류 발생.
...
-- NEXTVAL를 호출할 때마다 INCREMENT BY에 작성된 수 만큼 증가
SELECT SEQ_TEST_NO.NEXTVAL FROM DUAL; 
-- 처음 : 100
-- 1회 : 105
-- 2회 : 110
...
/* UPDATE에서 시퀀스 사용하기 */
-- '짱구'의 PK 컬럼 값을 SEQ_TEST_NO 시퀀스의 다음 생성 값으로 변경하기
UPDATE TB_TEST
SET TEST_NO = SEQ_TEST_NO.NEXTVAL
WHERE TEST_NAME = '짱구';
--> 짱구의 TEST_NO 값을 150까지 증가시키고나서 다시 한번 더 SEQ_TEST_NO.NEXTVAL 호출
-- ORA-08004: 시퀀스 SEQ_TEST_NO.NEXTVAL exceeds == MAXVALUE은 사례로 될 수 없습니다 
 --> MAXVALUE 150 보다 증가할 수 없다!!
SELECT * FROM TB_TEST;
--------------------------------
-- SEQUENCE 변경(ALTER) : START WITH 빼고 모두 변경 가능
/* [작성법]
...
-- SAQ_TEST_NO의 MASVALUE 값을 200으로 수정
...
-- TB_IDX_TEST 테이블에 샘플 데이터 100만개 삽입(PL(절차적 언어 SQL)/SQL 사용)
BEGIN
	FOR I IN 1..1000000
	LOOP
